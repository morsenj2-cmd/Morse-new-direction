  ...
  await db.execute(sql`
    ...
    SET
      weight = GREATEST(
        ...
        LEAST(
          ${MAX_WEIGHT},
          COALESCE(ut.weight, 1.0) - (
            GREATEST(0, EXTRACT(EPOCH FROM (${now}::timestamp - COALESCE(ut.last_interacted_at, ${now}::timestamp))) / 86400.0)
            * CASE WHEN COALESCE(ut.inferred, false) THEN ${INFERRED_DAILY_DECAY} ELSE ${EXPLICIT_DAILY_DECAY} END
          )
        )
      )
